...
Collections: `projects` (+ selects `categories` — **multi-select**, a project may belong to several ones, the first is the "main" one used for card badges —, `context` = `personal|school|professional`, `languages`), `project_translations` (unique per project+locale), `project_techs` (junction with `role` and 1-based `position` — PB treats 0 as blank on required numbers), `technologies` (lowercase hex `color` enforced by pattern, `group` select), `reviews` (flat `_fr`/`_en` columns). Select identifiers are stored in the DB; their fr/en labels live in `categories.ts` (`CATEGORIES`, `CONTEXTS`, `TECH_ROLES`) and `technologies.ts` (`GROUP_TITLES`).
